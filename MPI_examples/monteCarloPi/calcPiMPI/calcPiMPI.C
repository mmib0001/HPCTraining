...
int main(int argc, char** argv) {
   ...
   // Add codes here. Set up primary MPI routines
   ...
   MPI_Barrier(MPI_COMM_WORLD); // this line is for synchronisation 
   start = MPI_Wtime();
   processNumberInCircle = Toss(numProcessTosses, myRank);
   finish = MPI_Wtime();
   loc_elapsed = finish-start;

   // Add codes here. Combine the result of the processes
